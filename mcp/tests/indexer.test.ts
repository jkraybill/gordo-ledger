...
describe('HNSWIndexer', () => {
  ...
  // S347: RRF (Reciprocal Rank Fusion) tests
  describe('RRF Hybrid Search', () => {
    it('should normalize RRF scores to [0, 1] range', async () => {
      ...
      const sessions: SessionEntry[] = [
        {
          ...
          summary: 'Document with exact keywords',
          patterns: [],
          issues: [],
          signals: { success: true, failed: false, warning: false, ledTo: false, mixed: false, bigChange: false }
        },
        {
          id: 'semantic_only',
          date: '2025-01-02',
          content: 'unrelated terms here',
          summary: 'Document for semantic matching',
          patterns: [],
          issues: [],
          signals: { success: true, failed: false, warning: false, ledTo: false, mixed: false, bigChange: false }
        }
      ...
      // Embeddings: make first doc similar to query embedding, second doc different
      const embeddings = [
        new Array(1024).fill(0.8), // Similar to query
        new Array(1024).fill(0.2)  // Different from query
      ];
      await indexer.addDocuments(sessions, embeddings);

      const queryEmbedding = new Array(1024).fill(0.8); // Close to first doc
      const results = await indexer.hybridSearch(queryEmbedding, 'alpha beta', 10);
    ...
    it('should rank documents appearing in both retrievers higher (RRF fusion)', async () => {
      // Doc that should rank high in BOTH dense and BM25
      const sessions: SessionEntry[] = [
        {
          id: 'both_retrievers',
          date: '2025-01-01',
          content: 'target keyword target keyword semantic match',
          summary: 'High in both',
          patterns: [],
          issues: [],
          signals: { success: true, failed: false, warning: false, ledTo: false, mixed: false, bigChange: false }
        },
        ...
        },
        {
          id: 'bm25_only',
          date: '2025-01-03',
          content: 'target keyword but different embedding space',
          summary: 'BM25 match only',
          patterns: [],
          issues: [],
          signals: { success: true, failed: false, warning: false, ledTo: false, mixed: false, bigChange: false }
        }
      ...
      const embeddings = [
        new Array(1024).fill(0.9), // Close to query embedding (for RRF boost)
        new Array(1024).fill(0.85), // Also close (dense only)
        new Array(1024).fill(0.1)  // Far from query (BM25 only)
      ];
      await indexer.addDocuments(sessions, embeddings);

      const queryEmbedding = new Array(1024).fill(0.9);
      const results = await indexer.hybridSearch(queryEmbedding, 'target keyword', 10);

      // Document appearing in both should have highest score (RRF fusion benefit)
      expect(results.length).toBeGreaterThan(0);
      // The document with both dense and BM25 match should rank first
      // (This tests that RRF fusion is working correctly)
      expect(results[0].id).toBe('both_retrievers');
    });

    it('should pass default threshold check with normalized scores', async () => {
      const sessions: SessionEntry[] = [
        {
          id: 'good_match',
          date: '2025-01-01',
          content: 'important meeting notes from session',
      ...
      const embeddings = [new Array(1024).fill(0.9)];
      await indexer.addDocuments(sessions, embeddings);

      // Query with high similarity
      const queryEmbedding = new Array(1024).fill(0.9);
      const results = await indexer.hybridSearch(queryEmbedding, 'meeting notes', 10, 0.5);

      // With RRF normalization, a good match should pass threshold 0.5
      // If this fails, it means RRF scores aren't being normalized properly
      expect(results.length).toBeGreaterThan(0);
      expect(results[0].score).toBeGreaterThanOrEqual(0.5);
    });

    it('should filter results below threshold correctly', async () => {
      const sessions: SessionEntry[] = [
        {
          id: 'poor_match',
          date: '2025-01-01',
          content: 'completely unrelated document text',
          summary: 'Unrelated',
          patterns: [],
          issues: [],
          signals: { success: true, failed: false, warning: false, ledTo: false, mixed: false, bigChange: false }
      ...
      const embeddings = [new Array(1024).fill(0.1)]; // Very different from query
      await indexer.addDocuments(sessions, embeddings);

      const queryEmbedding = new Array(1024).fill(0.9);
      const results = await indexer.hybridSearch(queryEmbedding, 'specific search terms', 10, 0.8);

      // Poor match should be filtered out by high threshold
